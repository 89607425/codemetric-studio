...
function renderOoBarChart() {
  const host = byId('ooBarHost');
  if (!host) return;
  const metricKey = state.ooChartMetric || 'wmc';
  const rows = Array.isArray(state.rows) ? [...state.rows] : [];
  if (!rows.length) {
    host.innerHTML = '<div style="padding:16px;color:#9bb8db;">当前数据源没有类级明细，无法绘制柱状图。</div>';
    setStatus('oo-chart-status', '柱状图：暂无数据。', true);
    return;
  }
...
function bindEvents() {
  ...
  byId('oo-toggle-chart')?.addEventListener('click', () => {
    state.ooView = state.ooView === 'bar' ? 'table' : 'bar';
    syncOoTableOrChart();
  });

  byId('oo-chart-metric')?.addEventListener('change', (event) => {
    state.ooChartMetric = event.target.value;
    if (state.ooView === 'bar') {
      renderOoBarChart();
    }
  });
